...
The chart below shows the coefficients of a logistic regression estimating support for the measure. This is a simple model not used to generate the final predictions but which can be helpful in understanding the influence of various factors on voters' stances. Points on either extreme of the chart indicate that the predicitor has a greater influence on support or opposition to the policy. Points close to zero indicate the predictor has relatively little influence on opposition.
...
A cutoff `r max_f1$cutoff` generates the highest F1 score on the holdout data. However, based on the survey data, the ballot initiative may have more support than opposition in the jurisdiction. This may require setting a higher cutoff score to reach a greater number of people.

Using the cutoff of `r max_f1$cutoff`, `r scales::percent(max_f1$fnr)` of voters in the holdout data who supported the initiative were incorrectly identified as opposing while `r scales::percent(max_f1$tnr)` of those that actually opposed the measure were correctly identified. 
...
Additionally, there appears to be a group of older, progressive Democrats who support the policy as shown in the chart below. It may be helpful to focus on younger voters who are more likley to oppose the ballot measure.

...
Can we generate some clusters that might be good predictors?

```{r}

train_kmeans <- train %>% 
  filter(complete.cases(.)) %>% 
  select(income_pct, activist_pct, age) %>% 
  mutate_all(function(x) scale(as.numeric(as.factor(x))))

fviz_nbclust(train_kmeans, kmeans, method = "silhouette", k.max = 20)

kmod <- kmeans(train_kmeans, centers = 7, nstart = 25)
